...
import { generateCSV, createCSVResponse } from "@/lib/export-utils"
import { z } from "zod"
...
const SummaryExportSchema = z.object({
...
export async function POST(request: Request) {
  try {
    ...
    const params = SummaryExportSchema.parse(body)
    ...
    if (reportType === "birds") {
      ...
      groupCounts.forEach((count, key) => {
        ...
        const row: Record<string, string> = { count: count.toString() }
      ...
      // Create rows with column values + count
      ...
      const filename = `${reportType}-summary-${new Date().toISOString().split("T")[0]}`

      return createCSVResponse(csvContent, filename)
    }

    // For other report types, return empty CSV
    const csvContent = generateCSV(["No data"], [])
    return createCSVResponse(csvContent, `${reportType}-summary`)
  } catch (error) {
    return handleApiError(error, "POST /api/reports/summary/export")
  }
}
